        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          >
            ...
            <div className="hidden md:flex flex-wrap gap-1.5 mt-5">
              {channels.map((c) => (
                <span key={c.label} className="inline-flex items-center gap-1 text-[10px] font-medium text-muted-foreground bg-muted rounded-full px-2.5 py-1">
                  <c.icon className="h-3 w-3 text-foreground/50" />
                  {c.label}
                </span>
              ))}
            </div>
          ...
          {/* Phone Mock — below copy on mobile */}
          <motion.div
            initial={{ opacity: 0, x: -20 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true, amount: 0.2 }}
                ...
                <div className="rounded-[1.25rem] bg-alpine-dark overflow-hidden">
                  <div className="flex items-center justify-between px-4 pt-2.5 pb-1.5">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-[9px] text-secondary font-mono-tech">Online</span>
                    </div>
                  </div>
